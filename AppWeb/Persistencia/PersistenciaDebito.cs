        ...
        public static List<Debito> ListarXCliente(int oCI)
        {
            ...
            try
            {
                ...
                if (oReader.HasRows)
                {
                    while (oReader.Read())
                    {
                        Debito oDebito = new Debito(oCI, Convert.ToInt32(oReader["NroTarj"]), Convert.ToDateTime(oReader["fechaVencimiento"]),
        ...
        public static List<Debito> ListarVencidas()
        {
            ...
            try
            {
                ...
                if (oReader.HasRows)
                {
                    while (oReader.Read())
                    {
                        Debito oDebito = new Debito(Convert.ToInt32(oReader["ci"]), Convert.ToInt32(oReader["NroTarj"]), Convert.ToDateTime(oReader["fechaVencimiento"]),
                            Convert.ToInt32(oReader["pers"]), Convert.ToInt32(oReader["CantCuentAsoc"]), Convert.ToInt32(oReader["saldo"]));
